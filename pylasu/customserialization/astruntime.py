...
from pylasu.validation.validation import (Issue, IssueSeverity, IssueType,
                                          Result)
...
def unserialize_result(json_result, root_unserializer) -> Result:
    ...
    result.issues = (
        [unserialize_issue(issue) for issue in json_result["issues"]]
...
def unserialize_issue(json_issue) -> Issue:
    return Issue(
        ...
        severity=(
            unserialize_issue_severity(json_issue["severity"])
            if json_issue["severity"]
            else None
        ),
        position=(
            unserialize_position(json_issue["position"])
            if json_issue["position"]
            else None
        ),
    )
...
def check_type(json, expected_type):
    ...
    if json["#type"] != expected_type:
        raise Exception(
            "unexpected type, expected %s but found %s" % (expected_type, json["#type"])
        )
...
def unserialize_position(json_position):
    return Position(
        start=(
            unserialize_point(json_position["start"])
            if "start" in json_position
            else None
        ),
...
def unserialize_reference_by_name(json_reference_by_name):
    return ReferenceByName(
        name=(
            json_reference_by_name["name"] if "name" in json_reference_by_name else None
        )
    )
