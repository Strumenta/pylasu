...
import astor
from lionwebpython.language import (Concept, Containment, Feature, Interface,
                                    Language, Property)
...
def _identify_topological_deps(
    ...
    for c in classifiers:
        if isinstance(c, Concept):
            if (
                c.get_extended_concept()
                and c.get_extended_concept().get_id() in id_to_concept
            ):
    ...
    return ast.FunctionDef(
        ...
        args=ast.arguments(
            ...
            args=[
                ...
                ast.arg(
                    arg="concept", annotation=ast.Name(id="Concept", ctx=ast.Load())
                ),
            ],
            kwonlyargs=[],
            kw_defaults=[],
            ...
            ast.Expr(
                value=ast.Call(
                    ...
                    keywords=[
                        ...
                        ast.keyword(
                            arg="position",
                            value=ast.Name(id="position", ctx=ast.Load()),
                        ),
                        ast.keyword(
                            arg="concept", value=ast.Name(id="concept", ctx=ast.Load())
                        ),
                    ],
                )
            ),
...
def _generate_from_concept(classifier: Concept) -> ClassDef:
    bases = []
    if (
        classifier.get_extended_concept().id
        == StarLasuBaseLanguage.get_astnode(LionWebVersion.V2023_1).id
    ):
        if (
            len(
                [
                    ...
                    if i.get_language().get_name() != "com.strumenta.StarLasu"
                ]
            )
            == 0
        ):
...
def ast_generation(click, language: Language, output, language_name=str):
    ...
    import_node = ast.ImportFrom(
        module="pylasu.lwmodel", names=[ast.alias(name="ASTNode", asname=None)], level=0
    )
    ...
    import_model = ast.ImportFrom(
        module="pylasu.model", names=[ast.alias(name="Position", asname=None)], level=0
    )
    # from rpg.language import CONTROL_SPECIFICATION
    import_concepts = ast.ImportFrom(
        module=f"{language_name.lower()}.language",
        names=[
            ast.alias(name=to_snake_case(e.get_name()).upper(), asname=None)
