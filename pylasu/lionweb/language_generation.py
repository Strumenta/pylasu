...
def language_generation(click, language: Language, output, language_name: str):
    imports = [
        ...
        ast.ImportFrom(
            module="typing", names=[ast.alias(name="cast", asname=None)], level=0
        ),
        ast.ImportFrom(
        ...
        ast.ImportFrom(
            module="lionwebpython.lionweb_version",
            names=[ast.alias(name="LionWebVersion", asname=None)],
            level=0,
        ),
        ast.ImportFrom(
            module="lionwebpython.serialization.serialization_provider",
            names=[ast.alias(name="SerializationProvider", asname=None)],
            level=0,
        ),
        ast.ImportFrom(
            module="pylasu.lionweb.starlasu",
            names=[ast.alias(name="position_deserializer", asname=None)],
            level=0,
        ),
    ]
                ...
                items=[
                    ast.withitem(
                        context_expr=ast.Call(
                            ...
                            keywords=[
                                ast.keyword(
                                    arg="encoding", value=ast.Constant(value="utf-8")
                                )
                            ],
                        ),
    ...
    starlasu_assign = ast.Assign(
        targets=[ast.Name(id="STARLASU_LANGUAGE", ctx=ast.Store())],
        value=ast.Call(
            func=ast.Name(id="_load_starlasu", ctx=ast.Load()), args=[], keywords=[]
        ),
    )
    load_language_func = ast.FunctionDef(
        ...
        body=[
                ...
                items=[
                    ast.withitem(
                        context_expr=ast.Call(
                            ...
                            args=[
                                ast.Name(
                                    id=f"{language_name.lower()}_language_path",
                                    ctx=ast.Load(),
                                ),
                                ast.Constant(value="r"),
                            ],
            ...
            ast.Assign(
                targets=[
                    ast.Name(id=f"{language_name.lower()}_language", ctx=ast.Store())
                ],
                value=ast.Call(
                    func=ast.Name(id="cast", ctx=ast.Load()),
                    args=[
            ...
            # return rpg_language
            ast.Return(
                value=ast.Name(id=f"{language_name.lower()}_language", ctx=ast.Load())
            ),
        ],
        decorator_list=[],
        returns=ast.Name(id="Language", ctx=ast.Load()),
    ...
    language_assign = ast.Assign(
        ...
        value=ast.Call(
            func=ast.Name(id="_load_language", ctx=ast.Load()), args=[], keywords=[]
        ),
    )

    module = ast.Module(
        body=imports
        + [load_starlasu_func, starlasu_assign, load_language_func, language_assign],
        type_ignores=[],
    )
    ...
    for element in language.get_elements():
        if isinstance(element, Concept):
            assign = ast.Assign(
                targets=[
                    ast.Name(
                        id=to_snake_case(element.get_name()).upper(), ctx=ast.Store()
                    )
                ],
                value=ast.Call(
                    func=ast.Attribute(
