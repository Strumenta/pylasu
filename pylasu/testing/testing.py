...
def assert_asts_are_equal(
    ...
    if expected.node_type != actual.node_type:
        case.fail(
            f"{context}: expected node of type {expected.node_type}, "
    ...
    for expected_property in expected.properties:
        try:
            actual_property = next(
                filter(lambda p: p.name == expected_property.name, actual.properties)
            )
        ...
        if expected_property.is_containment:
            if expected_property.multiple:
                ...
                assert_single_properties_are_equal(
                    case,
                    expected_property,
                    expected_prop_value,
                    actual_prop_value,
...
def assert_single_properties_are_equal(
    case,
    expected_property,
    expected_prop_value,
    actual_prop_value,
...
def assert_multi_properties_are_equal(
    case,
    expected_property,
    expected_prop_value,
    actual_prop_value,
    ...
    # TODO IgnoreChildren
    case.assertEqual(
        actual_prop_value is None,
        expected_prop_value is None,
        f"{context}.{expected_property.name} nullness",
    )
    if actual_prop_value is not None and expected_prop_value is not None:
        case.assertEqual(
            len(actual_prop_value),
            len(expected_prop_value),
            f"{context}.{expected_property.name} length",
        )
    for expected_it, actual_it, i in zip(
        expected_prop_value, actual_prop_value, range(len(expected_prop_value))
    ):
        assert_asts_are_equal(
            case,
            expected_it,
